...
:::note 
The specific timing in steps 4 (sampling) and 5 (changing) depends on the SPI mode, which is defined by both `CPOL` **(Clock Polarity)** and `CPHA` **(Clock Phase)**.

The example shown here describes **SPI Mode 0** (`CPOL=0`, `CPHA=0`), where the clock is idle-low, data is sampled on the *rising edge* and new data is written on the *falling edge*. 
:::
...
SPI has 4 different modes which define when data is read or written. These modes can be configured through CPOL (Clock Polarity) and CPHA (Clock Phase). On the STM32U5, these settings are controlled by configuring the **"Programmable clock polarity and phase"** bits within the peripheral registers for one of the three available SPI instances.
...
| | | |
|-|-|-| 
| `CPOL` | Clock polarity | Defines the idle level of the clock when no data is being sent. |
| `CPHA` | Clock phase | Defines the clock edge used to read the data bit. |
...
Now we can create our SPI instance. The example uses the SPI1 channel, but others can be used. Being an asynchronous SPI, it also needs a pair of `DMA` channels, one that handles the *transmission* of data, and one that handles the *receiving* of data.

We can use any DMA channels that we have not used for another peripheral.
...
The STM32 Nucleo-U545RE-Q has three SPI channels. Each channel has a multiple sets of pins that can be used for `CLK`, `MOSI` and `MISO`.
...
To read the value of a register, the main device (the STM32U5 or RP2350) needs to first send a control byte containing the address of that register, with the 7th bit a `1`.
...
To write a value to a register, the main device (the STM32U5 or RP2350) needs to first send a control byte containing the address of that register, with the 7th bit a `0`.
...
let mut screen_spi_config = embassy_stm32::spi::Config::default();
screen_spi_config.frequency = Hertz(3_000_000);

let screen_rst = Output::new(p.PXn, Level::Low, Speed::Low);
...
:::note System Clock Configuration

By default, the microcontroller may run at a lower frequency (e.g., 16 MHz). While this is enough to make the display work, the drawing speed will be visibly slow.

The code below configures the **PLL (Phase Locked Loop)** to boost the system clock to 160 MHz.
...
This high frequency is necessary to:
- Process Graphics: Calculate pixel colors and fill buffers quickly.
- Drive SPI: Support high data transfer rates (high SPI frequencies) required for smooth frame updates. 
:::
...
Make sure to wire the pins on **J7** according to the pins chosen in the code and the picture below.

![screen_wiring](images/screen_wiring.jpg)
...
```
:::
- Read and print the rest of the values. (**2p**)
...
1. Did you notice that, when resting the board on the table, the acceleration over the **z** axis indicates the acceleration caused by Earth's gravity? Using this fact, if kept still, you can determine the absolute [pitch and roll](https://en.wikipedia.org/wiki/Aircraft_principal_axes) of the sensor. Create a small game where a ball rolls on the screen depending on the tilt. (**1p**)
...
Make sure to add the latest version of `micromath` to the project `Cargo.toml` dependencies.
:::
