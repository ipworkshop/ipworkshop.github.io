...
We can connect the EEPROM to the same I2C bus as the BMP390, therefore we can reuse the same `I2c` instance we [previously](./index.md#reading-and-writing-to-the-bmp390) initialized.
...
* Read the raw temperature value stored in the `DATA_3...DATA_5` register once a second, and print it to the terminal using the `defmt` macros. Details on how this can be performed can be found in this [subsection](./index.md#register-temp-0xfa0xfc) of the register map and in the [Reading a register](./index.md#reading-a-register) subsection. (**2p**)

3. Based on the raw temperature value previously determined, compute the actual temperature value using the **mock** calibration values provided bellow and the formula described in the [sections above](./index.md#temperature-computation-formula). (**1p**)
